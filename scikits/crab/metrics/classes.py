...
def check_sampling(sampling, num):
   ...
   ===========
    ...
    if isinstance(sampling, numbers.Number):
        sampling = SplitSampling(num, evaluation_fraction=sampling)

    return sampling


def check_cv(cross_validation, num):
    """Input checker utility for building a
   ...
   ===========
    ...
    if cross_validation is None:
        cross_validation = 3
    if isinstance(cross_validation, numbers.Number):
        cross_validation = KFold(num, cross_validation, indices=True)
...
class CfEvaluator(RecommenderEvaluator):
    ...
    def evaluate(self, recommender, metric=None, **kwargs):
        ...
        if metric not in EVALUATION_METRICS and metric is not None:
            raise ValueError(
                'metric %s is not recognized. valid keywords are %s' % (metric, list(EVALUATION_METRICS.keys())))
        ...
        if metric is None:
            ...
            mae, nmae, rmse = evaluation_error(real_preferences,
                                               ...
                                               recommender.model.maximum_preference_value(),
                                               recommender.model.minimum_preference_value())
            f1score = f1_score(real_arrays, relevant_arrays)
            recall = recall_score(real_arrays, relevant_arrays)
            precision = precision_score(real_arrays, relevant_arrays)

            return {'mae': mae, 'nmae': nmae, 'rmse': rmse, 'precision': precision,
    ...
    def evaluate_on_split(self, recommender, metric=None, cv=None, **kwargs):
        ...
        for train_idx, test_idx in cross_val:
            ...
            if metric in ['precision', 'recall', 'f1score']:
                ...
                recall = recall_score(real_arrays, relevant_arrays)
                precision = precision_score(real_arrays, relevant_arrays)
                permutation_scores_ir.append({'precision': precision, 'recall': recall, 'f1score': f1score})
